...
import { Plus, Trash2, Save, ArrowLeft, Calendar, User, FileText, IndianRupee, Package, Search, Info } from 'lucide-react';
...
const NewQuotation = () => {
  ...
  useEffect(() => {
    if (id && products.length > 0 && customers.length > 0) {
      ApiService.getById('quotations', id).then(response => {
        if (response) {
          const qtn = response.quotation || response;
          const qtnItems = response.items || [];
          const customer = customers.find(c => String(c.id) === String(qtn.customerId));
          ...
          setMaster({
            ...
            taxAmount: parseFloat(qtn.taxAmount) || 0,
            discountAmount: qtn.discountValue !== undefined && qtn.discountValue !== null ? parseFloat(qtn.discountValue) : (parseFloat(qtn.discountAmount) || 0),
            discountType: qtn.discountType || 'percent',
            grandTotal: parseFloat(qtn.grandTotal) || 0,
            totalDiscount: parseFloat(qtn.discountAmount) || 0,
            notes: qtn.notes || '',
            roundOff: 0,
            taxBreakdown: []
          });
          ...
          setChildren(qtnItems.map(item => {
            ...
            const actualDisc = parseFloat(item.discount) || 0;
            ...
            return {
              ...
              quantity: qty,
              freeQuantity: parseFloat(item.freeQuantity) || '',
              unit: item.unit,
              saleRate: rate,
              discount: actualDisc,
              discountType: 'amount',
              ...
              amount: rowSub,
              conversionFactor: parseFloat(item.conversionFactor) || 1,
  ...
  const handleMasterChange = (field, value) => {
    ...
    if (field === 'customerId') {
      if (!value) {
        setMaster(prev => ({ ...prev, customerBalance: 0, quotationNo: '', date: '' }));
        return;
      }

      const customer = customers.find(c => String(c.id) === String(value));
      ApiService.getAll('quotations/next-quotation-no').then(qtnData => {
        setMaster(prev => ({
          ...prev,
          customerBalance: customer?.balance || 0,
  ...
  const handleSaveQuotation = async () => {
    ...
    try {
      const payload = {
        ...
        date: master.date,
        validUntil: master.validUntil || null,
      ...
      let response;
      if (id) {
        response = await ApiService.update('quotations', id, payload);
      } else {
        response = await ApiService.add('quotations', payload);
      }
        ...
        >
          <div>
            <h2 style={{ fontSize: "18px", marginBottom: "1px" }}>
              {id ? "Edit Quotation" : "Professional Quotation"}
            </h2>
            <p style={{ fontSize: "12px", margin: 0 }}>
              {id ? `Edit Price Quote for ${master.quotationNo}` : "Krushi Seva Kendra Quotation System"}
            </p>
          </div>
          <button
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
            ...
            <div style={{ border: "1px solid var(--border-light)", borderRadius: "12px", overflow: "hidden", background: "#ffffff" }}>
              <div style={{ padding: "10px 15px", background: "#ffffff", borderBottom: "1px solid var(--border-light)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <button className="btn-agro btn-primary" onClick={addChildRow} style={{ height: "28px", padding: "0 10px", fontSize: "11px", background: "#8b5cf6" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 8fr) minmax(0, 4fr)", gap: "20px", marginTop: "20px" }}>
              <div style={{ padding: "20px", background: "#ffffff", borderRadius: "12px", border: "1px solid var(--border-light)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "20px" }}>
                  ...
                  <h3 style={{ fontSize: "14px", margin: "0", fontWeight: "800", color: "#1e293b", letterSpacing: "0.05em" }}>QUOTATION SETTINGS</h3>
                </div>
              ...
              <div style={{ padding: "20px", background: "#f5f3ff", borderRadius: "12px", border: "1px solid #ddd6fe" }}>
                <h3 style={{ fontSize: "14px", margin: "0 0 15px 0", fontWeight: "800", color: "#8b5cf6" }}>Quotation Summary</h3>
                <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                  ...
                  <div style={{ margin: "15px 0", borderTop: "1px dashed #ddd6fe" }}></div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", color: "#6d28d9" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: "10px", marginTop: "20px" }}>
                    ...
                    <button onClick={handleSaveQuotation} style={{ height: "42px", borderRadius: "8px", border: "none", background: "#8b5cf6", color: "#ffffff", fontWeight: "700", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", transition: "all 0.2s" }} onMouseOver={(e) => { e.target.style.background = "#7c3aed" }} onMouseOut={(e) => { e.target.style.background = "#8b5cf6" }}>
            ...
            <div style={{ marginTop: "20px", marginBottom: "15px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                <h3 style={{ fontSize: "13px", margin: 0, fontWeight: "700", color: "#8b5cf6" }}>
                  Tax Breakdown (CGST & SGST)
                </h3>
                <span style={{ background: '#f5f3ff', color: '#6d28d9', padding: '3px 10px', borderRadius: '20px', fontSize: '10px', fontWeight: '800', border: '1px solid #ddd6fe' }}>
                  INTRA-STATE SALE
                </span>
              </div>
